...
const { typeRights } = require('../../src/config/types');
...
describe('Auth routes', () => {
  describe('POST /v1/auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({ name: newUser.name, email: newUser.email, type: 'admin', isEmailVerified: false });
...
describe('Auth middleware', () => {
  ...
  test('should call next with no errors if user has required rights', async () => {
    ...
    await auth(...typeRights.get('admin'))(req, httpMocks.createResponse(), next);
