...
describe('User routes', () => {
  describe('POST /v1/users', () => {
    ...
    test('should return 201 and successfully create new user if data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({ name: newUser.name, email: newUser.email, type: newUser.type, isEmailVerified: false });
    });
    ...
    test('should be able to create an admin as well', async () => {
      ...
      expect(res.body.type).toBe('superAdmin');

      const dbUser = await User.findById(res.body.id);
      expect(dbUser.type).toBe('superAdmin');
    });
    ...
    test('should return 400 error if type is neither superAdmin nor admin', async () => {
      await insertUsers([admin]);
      newUser.type = 'invalid';
  ...
  describe('GET /v1/users', () => {
    ...
    test('should correctly sort the returned array if multiple sorting criteria are specified', async () => {
      ...
      const res = await request(app)
        .get('/v1/users')
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .query({ sortBy: 'type:desc,name:asc' })
        .send()
        .expect(httpStatus.OK);
  ...
  describe('PATCH /v1/users/:userId', () => {
    test('should return 200 and successfully update user if data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({ name: updateBody.name, email: updateBody.email, type: 'admin' });
    });
